...
export class studentdataController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/studentdata/:id', this.GpDelete);
this.router.get('/studentdata/get/search', this.GpSearch);
this.router.put('/studentdata/get/update', this.GpSearchForUpdate);
this.router.put('/studentdata', this.GpUpdate);
this.router.get('/studentdata/:id', this.GpGetNounById);
this.router.get('/studentdata', this.GpGetAllValues);
this.router.post('/studentdata', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into studentdataController.ts: GpDelete');
        new ApiAdapter().delete(Constant.STUDENTDETAILSURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.STUDENTDETAILSURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.STUDENTDETAILSURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.STUDENTDETAILSURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
